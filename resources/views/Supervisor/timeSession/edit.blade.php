...
@section('content')
    ...
    <h1 class="main-title">تعديل حصة في الجدول</h1>
    ...
    <div class="card card-soft">
        <div class="card-body">
            <form method="POST"
                  action="{{ route('timetable.sessions.update', [$timetable->id, $timetableSession->id]) }}"
                  class="row g-3">
                @csrf
                @method('PUT')
                    ...
                    <select id="timeslot_id"
                        ...
                        @foreach($timeslots as $slot)
                            <option value="{{ $slot->id }}"
                                {{ old('timeslot_id', $timetableSession->timeslot_id) == $slot->id ? 'selected' : '' }}>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select id="subject_id"
                        ...
                        @foreach($subjects as $sub)
                            <option value="{{ $sub->id }}"
                                {{ old('subject_id', $timetableSession->subject_id) == $sub->id ? 'selected' : '' }}>
                                {{ $sub->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select id="teacher_id"
                        ...
                        @foreach($teachers as $t)
                            <option value="{{ $t->id }}"
                                {{ old('teacher_id', $timetableSession->teacher_id) == $t->id ? 'selected' : '' }}>
                                {{ $t->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select id="section_id"
                        ...
                        @foreach($sections as $s)
                            <option value="{{ $s->id }}"
                                {{ old('section_id', $timetableSession->section_id) == $s->id ? 'selected' : '' }}>
                                {{ $s->name }}
                            </option>
                        @endforeach
                    </select>
