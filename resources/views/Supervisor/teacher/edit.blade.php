...
@section('title','تعديل بيانات معلّم')

@section('content')

    <h1 class="main-title">تعديل بيانات معلّم</h1>

    <div class="card card-soft">
        <div class="card-body">
            <form method="POST" action="{{ route('teacher.update', $teacher->id) }}" class="row g-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select id="user_id"
                        ...
                        @foreach($user as $u)
                            <option value="{{ $u->id }}"
                                {{ old('user_id', $teacher->user_id) == $u->id ? 'selected' : '' }}>
